...
def downloadOneVideo(name,title,url,timestamp):
    ...
    global VideoCount
    try:
        os.chdir(os.path.join(os.path.abspath('..'),'autoDownloader'))
        ...
        if(os.path.exists(os.path.join("download",name))==0):
            os.makedirs(os.path.join("download",name))
        with open(os.path.join("download",name,title+"_"+str(timestamp)+".mp4"), "wb") as code:
            code.write(webvideopage.content)
        conn.execute("UPDATE rssData set downloaded = 1  where rssName='{}' and title='{}'".format(name,title))
        conn.commit()
